...
/**
 * Test Case 8.1 (PayPal) — payment failure via Dispatch Order API block,
 * then retry. Mirrors credit-card/08-riskified.spec.ts but drives PayPal
 * for both the failing and succeeding attempts.
 */
test.describe('PP · Riskified · Dispatch Order failure & retry', () => {
  test('8.1 Blocking dispatch order fails PayPal, unblock and retry succeeds', async ({
    page,
    flow,
    paymentPage,
    confirmationPage,
    ...
    await confirmationPage.captureScreenshot({ testId: '8.1-pp-riskified-dispatch-block', testInfo });
